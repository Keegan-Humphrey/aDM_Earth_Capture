...
Vmantle = (4\[Pi])/3 (("rE")^3-("rcore")^3)/.Constants`EarthRepl;
Vcore = (4\[Pi])/3 ("rcore")^3/.Constants`EarthRepl;
...
FeNucCoeffs = <|"name"->"Fe","a"->{11.7695,7.3573,3.5222,2.3045},"b"->{4.7611,0.3072,15.3535,76.8805},"c"->1.0369,"Z"->26,"A"->56,"rn"->rn[56],"mN"->56 1.66 10^-27,"nI"->1.402 10^29,"\[Beta]"->Constants`\[Beta]core,"Vol"->Vcore|>;
SiNucCoeffs = <|"name"->"Si","a"->{6.2915,3.0353,1.9891,1.541},"b"->{2.4386,32.3337,0.6785,81.6937},"c"->1.1407,"Z"->14,"A"->28,"rn"->rn[28],"mN"->28 1.66 10^-27,"nI"->4.995 10^28,"\[Beta]"->Constants`\[Beta]crust,"Vol"->Vmantle|>;
MgNucCoeffs = <|"name"->"Mg","a"->{5.4204,2.1735,1.2269,2.3073},"b"->{2.8275,79.2611,0.380,7.1937},"c"->0.8584,"Z"->12,"A"->24,"rn"->rn[24],"mN"->24 1.66 10^-27,"nI"->4.362 10^28,"\[Beta]"->Constants`\[Beta]crust,"Vol"->Vmantle|>;
ONucCoeffs = <|"name"->"O","a"->{3.0485,2.2868,1.5463,0.867},"b"->{13.2771,5.7011,0.3239,32.9089},"c"->0.2508,"Z"->8,"A"->16,"rn"->rn[16],"mN"->16 1.66 10^-27,"nI"->1.435 10^29,"\[Beta]"->Constants`\[Beta]crust,"Vol"->Vmantle|>;
