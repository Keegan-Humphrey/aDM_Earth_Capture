...
def dRdomegadk_electron(self,omega,k,sigmae=1e-38,withscreening=True, method="grid",vdist="halo"):
    """
    ...
    """
    etav_val = self.etav(self.vmin(omega,k)) if vdist == "halo" else self.etav_disk(self.vmin(omega,k)) #if vdist == "disk" else 0
...
def dRdomega_electron(self,omega, sigmae=1e-38, kcut=0, withscreening=True, method="grid",vdist="halo"):
    """
    ...
    for i in range(len(omega)):
        ...
        dRdomega[i] = self.eVtoInvYr * integrate.quad(lambda x: self.dRdomegadk_electron(omega[i],x,sigmae, \
                    withscreening=withscreening,method=method,vdist=vdist)/self.eVtoInvYr, kmin, kmax, limit=50)[0] 
                     # units of 1/kg/yr/eV
...
def R_electron(self,threshold=-1.0,Emax=-1.0,sigmae=1e-38, kcut = 0, withscreening=True, method="grid",vdist="halo"):
    """
    ...
    return np.trapz(self.dRdomega_electron(olist,sigmae=sigmae,kcut=kcut, \
        withscreening=withscreening,method=method,vdist=vdist), x=olist)
